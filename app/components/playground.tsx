...
export default function Playground({}: PlaygroundProps) {
  ...
  // State for the custom cursor in Grid View
                ...
                return (
                  <div
                    ...
                    className={`
                      ...
                      w-[85vw] md:w-80% md:h-75vh
                      aspect-[4/3]
                      max-w-[1011px] max-h-[754px]
          ...
          <>
            ...
            <div className="px-50 columns-2 md:columns-3 gap-6 space-y-6 animate-in fade-in duration-500">
              {PLAYGROUND_ITEMS.map((item) => (
                <div
                  key={item.id}
                  // Toggle cursor state on enter/leave
                  onMouseEnter={() => setIsGridCursorActive(true)}
                  onMouseLeave={() => setIsGridCursorActive(false)}
                  // 'cursor-none' hides the default pointer so we only see the CustomCursor
                  className="break-inside-avoid overflow-hidden flex flex-col gap-3 shadow-[0px_4px_24px_0px_rgba(0,0,0,0.08)] border border-transparent hover:border-primary-text hover:-translate-y-1 transition-all duration-300 cursor-none"
                >
                  <Image
                    src={item.src}
                    alt={item.title}
